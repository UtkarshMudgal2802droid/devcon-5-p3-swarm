import { BatchId, PublicKey, Reference } from '@ethersphere/core-sdk';
import type { BeeRequestOptions, GetGranteesResult, GranteesResult } from '../types';
import type { BeeContext } from './context';
/**
 * Grantee (access control) operations.
 *
 * Accessed as `bee.grantee`.
 */
export declare class Grantee {
    private readonly context;
    constructor(context: BeeContext);
    /**
     * Creates grantees for a postage batch.
     *
     * @param postageBatchId
     * @param grantees Public keys of the grantees.
     * @param requestOptions Options for making requests, such as timeouts, custom HTTP agents, headers, etc.
     */
    create(postageBatchId: BatchId | Uint8Array | string, grantees: PublicKey[] | Uint8Array[] | string[], requestOptions?: BeeRequestOptions): Promise<GranteesResult>;
    /**
     * Retrieves the grantees for a given reference.
     *
     * @param reference The reference.
     * @param requestOptions Options for making requests, such as timeouts, custom HTTP agents, headers, etc.
     */
    get(reference: Reference | Uint8Array | string, requestOptions?: BeeRequestOptions): Promise<GetGranteesResult>;
    /**
     * Updates the grantees of a specific reference and history.
     *
     * @param postageBatchId The ID of the postage batch.
     * @param reference The reference.
     * @param history The history.
     * @param grantees The grantees to add and/or revoke.
     * @param requestOptions Options for making requests, such as timeouts, custom HTTP agents, headers, etc.
     */
    patch(postageBatchId: BatchId | Uint8Array | string, reference: Reference | Uint8Array | string, history: Reference | Uint8Array | string, grantees: {
        add?: PublicKey[] | Uint8Array[] | string[];
        revoke?: PublicKey[] | Uint8Array[] | string[];
    }, requestOptions?: BeeRequestOptions): Promise<GranteesResult>;
}
