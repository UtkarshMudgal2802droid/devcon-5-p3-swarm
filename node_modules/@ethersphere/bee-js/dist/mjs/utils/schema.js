...
export { HexStringSchema } from "./hex-schema.js";
const fnField = z.any().refine(v => z.function().safeParse(v).success, 'expected a function');
const integerStringField = z.any().transform(v => {
  if (z.bigint().safeParse(v).success) v = String(v);
  return z.string().regex(/^-?\d+$/, 'expected integer string').parse(v);
});
const nonNegativeIntegerStringField = z.any().transform(v => {
  if (z.bigint().safeParse(v).success) v = String(v);
  return z.string().regex(/^\d+$/, 'expected non-negative integer string').parse(v);
});
...
export const DownloadOptionsSchema = z.object({
  ...
  actPublisher: z.optional(z.any().transform(v => new PublicKey(v))),
  actHistoryAddress: z.optional(z.any().transform(v => new Reference(v))),
