...
const ChequeSchema = z.object({
  ...
  payout: z.string().transform(s => BZZ.fromPLUR(asNumberString(s, {
    name: 'payout'
  })))
});
const CashoutResultSchema = z.object({
  recipient: z.string(),
  lastPayout: z.string().transform(s => BZZ.fromPLUR(asNumberString(s, {
    name: 'lastPayout'
  }))),
  bounced: z.boolean()
});
...
export const GetChequebookBalanceResponse = z.object({
  availableBalance: z.string().transform(s => BZZ.fromPLUR(asNumberString(s, {
    name: 'availableBalance'
  }))),
  totalBalance: z.string().transform(s => BZZ.fromPLUR(asNumberString(s, {
    name: 'totalBalance'
  })))
});
export const GetLastCashoutActionResponse = z.object({
  peer: z.string(),
  uncashedAmount: z.string().transform(s => BZZ.fromPLUR(asNumberString(s, {
    name: 'uncashedAmount'
  }))),
  transactionHash: z.string().nullish().transform(v => v ?? null),
