...
export const GetChainStateResponse = z.object({
  block: z.number(),
  chainTip: z.number(),
  totalAmount: z.string().transform(s => asNumberString(s, {
    name: 'totalAmount'
  })),
  currentPrice: z.string().transform(Number).transform(s => normalizeCurrentPrice(s)),
  minimumValidityBlocks: z.number().optional()
});
export const GetWalletBalanceResponse = z.object({
  bzzBalance: z.string().transform(s => BZZ.fromPLUR(asNumberString(s, {
    name: 'bzzBalance'
  }))),
  nativeTokenBalance: z.string().transform(s => DAI.fromWei(asNumberString(s, {
