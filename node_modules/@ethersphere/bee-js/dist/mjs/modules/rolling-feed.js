import { BatchId, keccak256, numberToUint64, Reference, Topic } from '@ethersphere/core-sdk';
import { BeeError, Bytes } from "../index.js";
import { fetchLatestFeedUpdate, probeFeed } from "../api/feed.js";
import { uploadSingleOwnerChunkWithWrappedChunk } from "../chunk/soc.js";
import { downloadFeedUpdate, downloadFeedUpdateAsCAC, updateFeedWithPayload, updateFeedWithReference } from "../feed/index.js";
import { makeFeedIdentifier } from "../feed/identifier.js";
import { BeeResponseError } from "../utils/error.js"; // RollingFeedReader.downloadPayload/downloadReference only ever fall back one period, so
...
export class RollingFeedWriter {
  ...
  async uploadPayload(postageBatchId, payload, options) {
    ...
    const {
      ...
      nextTopic,
      mirrorOptions
    } = this.currentAndNextTopics(options);
    const [result] = await Promise.all([updateFeedWithPayload(requestOptions, this.signer, currentTopic, payload, stamp, options), updateFeedWithPayload(requestOptions, this.signer, nextTopic, payload, stamp, mirrorOptions)]);
    return result;
  }
  async uploadReference(postageBatchId, reference, options) {
    ...
    const [result] = await Promise.all([updateFeedWithReference(requestOptions, this.signer, currentTopic, reference, stamp, options), updateFeedWithReference(requestOptions, this.signer, nextTopic, reference, stamp, mirrorOptions)]);
    return result;
  }
  /**
  ...
  async catchUp(postageBatchId, periodIdx, maxBackfill = DEFAULT_MAX_BACKFILL) {
    ...
    const {
      feedIndex: sourceIndex
    } = await probeFeed(requestOptions, owner, sourceTopic);
